        ...
        listener: (context, state) {
          if (state is AuthLoding) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('creating_account'.tr()), backgroundColor: Colors.blueAccent),
            );
          } else if (state is Authanticated) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('account_created'.tr()), backgroundColor: Colors.green),
            );
            Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => HomePage()));
          } else if (state is AuthError) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('${'error_occurred'.tr()}: ${state.message}'), backgroundColor: Colors.red),
            );
          }
        },
        child: Padding(
          ...
          child: Center(
            child: SingleChildScrollView(
              child: Column(
                children: [
                  SizedBox(width: 150, height: 150, child: Center(child: Image.asset("assets/images/shcool_logo.png"))),
                  ...
                  const SizedBox(height: 20),
                  buildTextField(controller: gradeController, label: 'grade'.tr()),
                  const SizedBox(height: 20),
                  buildTextField(controller: ageController, label: 'age'.tr()),
                  const SizedBox(height: 20),
                  buildTextField(controller: addressController, label: 'address'.tr()),
                  const SizedBox(height: 20),
                  buildTextField(controller: conditionController, label: 'condition'.tr()),
                    ...
                    builder: (context, state) {
                      ...
                      return ElevatedButton(
                        ...
                        style: ElevatedButton.styleFrom(
                          ...
                          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8), side: const BorderSide(color: Colors.white)),
                        ),
                        child: isLoading
                            ? const SizedBox(
                          height: 20,
                          ...
                          child: CircularProgressIndicator(strokeWidth: 2, valueColor: AlwaysStoppedAnimation<Color>(Colors.white)),
                            ...
                            : Text('register'.tr(), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                      );
                    },
                  ),
                  const SizedBox(height: 30),
